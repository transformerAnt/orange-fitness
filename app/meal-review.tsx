...
export default function MealReviewScreen() {
  const { user } = useUser();
  const { imageUri, mealType } = useLocalSearchParams<{ imageUri: string; mealType: string }>();
  ...
  useEffect(() => {
    const run = async () => {
      ...
      const response = await apiPost<FoodAnalysis>('/food/analyze', { imageBase64: dataUrl });
      setLoading(false);
      if (response.error) {
    ...
    <SafeAreaProvider style={{ flex: 1, backgroundColor: Design.colors.background }}>
      <ScrollView style={{ flex: 1, backgroundColor: Design.colors.background }}>
        <View style={{ paddingHorizontal: Design.spacing.lg, paddingTop: 20, paddingBottom: 40 }}>
          ...
          <Text style={{ color: Design.colors.ink, fontSize: 22, fontFamily: Design.typography.fontBold }}>
            Meal Review
          </Text>
          <Text style={{ color: Design.colors.muted, fontSize: 12, marginTop: 4 }}>
            {mealType || 'Meal'}
          </Text>
          ...
          {imageUri ? (
            <Image
              source={{ uri: imageUri }}
              style={{ width: '100%', height: 240, borderRadius: 18, marginTop: 16 }}
            />
          ) : null}
          ...
          <View
            ...
            <Text style={{ color: Design.colors.muted, fontSize: 12, marginTop: 8 }}>
              {loading ? 'Analyzing...' : `${Math.round(totals.calories)} kcal`}
            </Text>
            {!loading ? (
              <View style={{ marginTop: 12 }}>
                {[
                  { label: 'Protein', value: totals.protein, unit: 'g', color: '#4F46E5' },
                  { label: 'Carbs', value: totals.carbs, unit: 'g', color: '#22C55E' },
          ...
          {analysis?.items?.length ? (
            <View
              ...
              {analysis.items.map((item) => (
                <Text key={item.name} style={{ color: Design.colors.muted, fontSize: 12, marginTop: 6 }}>
            ...
            <View
              ...
              <Text style={{ color: Design.colors.muted, fontSize: 12, marginTop: 6 }}>{analysis.raw}</Text>
            </View>
          ) : null}
          ...
          <Pressable
            ...
            style={{
              ...
              paddingVertical: 12,
              borderRadius: 12,
              backgroundColor: loading || saving || !analysis ? Design.colors.line : Design.colors.accent,
              alignItems: 'center',
            }}>
